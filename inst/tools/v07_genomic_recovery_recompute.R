...
v07_read_raw <- function(out_dir, tier) {
  ...
  if (!dir.exists(root)) {
    v07_abort("missing raw result directory: %s", root)
  }
  ...
  if (!length(paths)) {
    v07_abort("no raw result files under %s", root)
  }
  ...
  attr(out, "raw_paths") <- normalizePath(
    paths,
    winslash = "/",
    mustWork = TRUE
  )
...
v07_as_numeric <- function(x, field) {
  ...
  if (any(bad)) {
    v07_abort("field %s contains nonnumeric values", field)
  }
  out
}
...
v07_normalize_raw <- function(raw) {
  ...
  if (any(!conv %in% c("true", "false"))) {
    v07_abort("converged must be true or false")
  }
...
v07_keys <- function(x) {
  paste(x$tier, x$cell_id, format(x$seed, scientific = FALSE), sep = "\r")
}

v07_assert_unique_keys <- function(x, label) {
  keys <- v07_keys(x)
  if (anyDuplicated(keys)) {
    v07_abort("duplicate tier/cell/seed key in %s", label)
  }
  invisible(TRUE)
}
...
v07_assert_sha256 <- function(x, field) {
  if (any(!grepl("^[[:xdigit:]]{64}$", x))) {
    v07_abort(
      "%s must contain lowercase or uppercase SHA-256 hex digests",
      field
    )
  }
}

v07_validate_inputs <- function(
  raw,
  manifest,
  tier,
  ...
  if (
    any(vapply(id_hashes_by_n, function(x) length(unique(x)) != 1L, logical(1)))
  ) {
...
v07_validate_disjoint_seeds <- function(pilot_manifest, confirm_manifest) {
  ...
  overlap <- intersect(
    scientific_key(pilot_manifest),
    scientific_key(confirm_manifest)
  )
  if (length(overlap)) {
    v07_abort("pilot and confirmation seed blocks overlap")
  }
  invisible(TRUE)
}
...
v07_recompute_summary <- function(
  raw,
  manifest,
  tier,
  expected_cells = v07_expected_cells
  ...
  for (cell in expected_cells) {
    ...
    finite <- is.finite(cr$estimate_sigma_g2) &
      is.finite(cr$estimate_sigma_e2) &
      is.finite(cr$estimate_ratio)
    ...
    if (nrow(truths) != 1L) {
      v07_abort("truth varies within cell %s", cell)
    }
    targets <- list(
    ...
    cell_status <- if (tier == "pilot" && rate < 0.95) {
      ...
      all(stats_rows$target_pass) && rate >= 0.95 && wilson[[1L]] >= 0.90
    ) {
      "PASS"
    } else {
      "FAIL"
...
v07_compare_summary <- function(recomputed, julia_summary, tolerance = 1e-10) {
  ...
  julia_summary <- julia_summary[
    match(key(recomputed), key(julia_summary)),
    ,
    drop = FALSE
  ]
  numeric_fields <- c(
  ...
  for (field in numeric_fields) {
    ...
    if (!v07_equal_numeric(lhs, rhs, tolerance)) {
  ...
  for (field in character_fields) {
    if (
      !identical(
        as.character(recomputed[[field]]),
        as.character(julia_summary[[field]])
      )
    ) {
      v07_abort("summary mismatch in %s", field)
    }
...
v07_sha256 <- function(path) {
  ...
  if (!is.null(status) && status != 0L) {
    v07_abort("SHA-256 command failed for %s", path)
  }
  strsplit(output[[1L]], "[[:space:]]+")[[1L]][[1L]]
}

v07_raw_lock_path <- function(out_dir, tier) {
  if (!tier %in% c("pilot", "confirm")) {
    v07_abort("tier must be pilot or confirm")
  }
  file.path(out_dir, paste0(tier, "_raw_sha256.tsv"))
}

v07_tier_raw_files <- function(out_dir, tier) {
  root <- file.path(out_dir, "raw", tier)
  sort(list.files(
    root,
    pattern = "\\.tsv$",
...
v07_write_raw_lock <- function(
  out_dir,
  tier,
  path = v07_raw_lock_path(out_dir, tier)
) {
  if (file.exists(path)) {
    v07_abort("raw campaign checksum lock already exists; refusing to reseal")
  }
  files <- v07_tier_raw_files(out_dir, tier)
  if (!length(files)) {
    v07_abort("no raw %s TSV files to seal", tier)
  }
  ...
  lock <- data.frame(
    path = rel,
    sha256 = vapply(files, v07_sha256, character(1)),
    stringsAsFactors = FALSE
  )
...
v07_verify_raw_lock <- function(
  ...
  current_paths <- sub(
    ...
    "",
    normalizePath(
      v07_tier_raw_files(out_dir, tier),
      winslash = "/",
      mustWork = TRUE
    )
  )
  if (!identical(sort(as.character(lock$path)), sort(current_paths))) {
    v07_abort("%s raw checksum lock file set differs from current files", tier)
  }
...
v07_run_gate <- function(out_dir, tier, tolerance = 1e-10, verify_lock = TRUE) {
  manifest_name <- if (tier == "pilot") {
    "pilot_manifest.tsv"
  } else {
    "confirmation_manifest.tsv"
  }
  raw <- v07_read_raw(out_dir, tier)
  manifest <- v07_read_tsv(
    file.path(out_dir, manifest_name),
    v07_manifest_columns
  )
  if (verify_lock) {
    v07_verify_raw_lock(out_dir, tier)
  }
  if (
    file.exists(file.path(out_dir, "pilot_manifest.tsv")) &&
      file.exists(file.path(out_dir, "confirmation_manifest.tsv"))
  ) {
    v07_validate_disjoint_seeds(
      v07_read_tsv(
        file.path(out_dir, "pilot_manifest.tsv"),
        v07_manifest_columns
      ),
      v07_read_tsv(
        file.path(out_dir, "confirmation_manifest.tsv"),
        v07_manifest_columns
      )
    )
  }
  ...
  julia <- v07_read_tsv(
    file.path(out_dir, paste0(tier, "_summary.tsv")),
    v07_summary_columns
  )
...
v07_main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (is.null(out_dir) || !tier %in% c("pilot", "confirm")) {
    v07_abort(
      "usage: Rscript tools/v07_genomic_recovery_recompute.R --out-dir=PATH --tier=pilot|confirm [--write-lock=true]"
    )
  }
  write_lock <- identical(
    tolower(v07_option(args, "write-lock", "false")),
    "true"
  )
  if (write_lock) {
    v07_write_raw_lock(out_dir, tier)
  }
  result <- v07_run_gate(out_dir, tier)
  message(sprintf(
    "v0.7 %s independent R recomputation: PASS (%d summary rows)",
    tier,
    nrow(result)
  ))
}
