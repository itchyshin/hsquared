...
Use this path when you already have a single-step inverse relationship
...
Use this path when you have a pedigree for all animals and a marker
matrix for the genotyped subset. Marker rows must be named by animal ID.
Phenotyped animals do not all need marker rows; ungenotyped animals are
the point of single-step.
...
The engine builds the pedigree inverse, dense pedigree relationship,
genomic relationship for the genotyped subset, and the single-step
precision `H^-1`. Construction knobs (`tau`, `omega`, `blend_weight`,
`ridge`) exist for validation work but are not comparator-validated
defaults.
