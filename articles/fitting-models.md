...
The remaining models below are **opt-in**: they are reached through
`engine = "julia"` with an explicit `target`. They are all REML-only,
experimental, and not the public default. Four of them are **covered at
validation scale** — each passed a pre-declared 48-seed recovery gate
plus a same-estimand REML comparator: the common-environment two-effect
model, arbitrary-N multi-effect, random regression k = 2, and the
direct–maternal 2×2 G model. Covered at validation scale means
experimental and not production; none is a default fit. The remaining
opt-in models
([`validation_status()`](https://itchyshin.github.io/hsquared/reference/validation_status.md)
rows `partial`) have not yet passed a comparator- or
known-truth-validated gate.
...
[`validation_status()`](https://itchyshin.github.io/hsquared/reference/validation_status.md)
may be relied on as working. The four opt-in models listed as covered
above (`two_effect` common-env leg, `multi_effect`, `random_regression`,
`direct_maternal`) are covered at validation scale. The remaining opt-in
models (`repeatability`, the maternal two-effect leg, genomic, SNP-BLUP,
single-step, multivariate) remain `partial` until their `HSquared.jl`
gates and external comparators are signed off.
Structured/factor-analytic multivariate covariance, non-Gaussian/GLLVM,
