...
G_t <- as.matrix(read.csv(
  file.path(dir, "expected_genomic_relationship.csv"),
  row.names = 1
))
Gi_t <- as.matrix(read.csv(
  file.path(dir, "expected_genomic_precision.csv"),
  row.names = 1
))
...
res(
  "max|solve(G_ref) - fixture Ginv|:",
  format(max(abs(solve(G_ref) - Gi_t)), digits = 3)
)

cat("\n== external: AGHmatrix::Gmatrix VanRaden (sample-estimated p) ==\n")
G_agh <- Gmatrix(SNPmatrix = M, method = "VanRaden", ploidy = 2)
res(
  "max|AGHmatrix G - fixture G| (sample p != supplied p):",
  format(max(abs(G_agh - G_t)), digits = 3)
)
cat(
  "  (AGHmatrix re-estimates p from the sample; a direct match is NOT expected.\n"
)
...
cat(
  "   construction FORMULA by feeding the engine's VanRaden formula AGHmatrix's\n"
)
...
res(
  "max|base-R VanRaden(p_AGH) - AGHmatrix G|:",
  format(max(abs(G_base_aghp - G_agh)), digits = 3)
)
...
res(
  "max|MME GBLUP - fixture gblup|:",
  format(max(abs(u_mme - gebv_t$gblup)), digits = 3)
)
...
res(
  "rrBLUP GBLUP vs SNP-BLUP GEBV max|diff| (equivalence):",
  format(max(abs(gebv_rr - gebv_from_me)), digits = 3)
)
res(
  "rrBLUP GBLUP GEBV cor with fixture gblup:",
  sprintf("%.6f", cor(gebv_rr, gebv_t$gblup))
)
res(
  "rrBLUP marker-effect cor with fixture effects:",
  sprintf("%.6f", cor(me_rr, me_t$effect))
)
res(
  "rrBLUP REML Vu / Ve (ratio):",
  sprintf("%.4f / %.4f (%.3f)", fit_g$Vu, fit_g$Ve, fit_g$Ve / fit_g$Vu)
)
...
fm <- BGLR(
  y = y,
  ETA = list(list(K = G_ref, model = "RKHS")),
  nIter = 12000,
  burnIn = 2000,
  verbose = FALSE,
  saveAt = sa
...
res(
  "BGLR posterior-mean GEBV cor with fixture gblup:",
  sprintf("%.6f", cor(gebv_bglr, gebv_t$gblup))
)

cat("\n== session ==\n")
for (pk in c("AGHmatrix", "rrBLUP", "BGLR")) {
  res(pk, as.character(packageVersion(pk)))
}
