...
| opt-in supplied-variance Henderson MME bridge | partial | local JuliaCall tests against sibling `HSquared.jl` compare the R validation fixture with Julia `henderson_mme()` through `hsquared()` | Requires `hs_control(engine = "julia", engine_control = list(target = "henderson_mme", variance_components = ...))`; supplied-variance validation path only, no optimizer or log-likelihood. |
