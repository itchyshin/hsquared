...
| `hsquared_fit` object/extractors | partial | local tests over internal mock fit results and opt-in tiny Julia result | PEV/reliability extractor contract exists; `fitted()` and `residuals()` work when predictions and response values are present; the opt-in bridge enriches tiny local Julia results from exported dense validation extractors when available. |
| `hs_data()` container | partial | local tests over phenotype, pedigree, genotype, expression, pedigree status, marker-map validation, genotype-marker alignment, ID overlap summaries, marker-status summaries, and v0.1 parser use including default pedigree lookup for `animal(1 \| id)` | No file-backed storage, genotype parsing, marker imputation, marker scanning, or automatic genotype/omics model construction yet. |
