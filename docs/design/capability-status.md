...
| `hs_data()` container | partial | local tests over phenotype, pedigree, genotype, expression, marker-map validation, genotype-marker alignment, ID overlap summaries, and v0.1 parser use | No file-backed storage, genotype parsing, marker imputation, marker scanning, or automatic genotype/omics model construction yet. |
