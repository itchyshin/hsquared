...
| `data_status()` | partial | local tests over `hs_data()` diagnostics | Reports data-container component, ID-overlap, pedigree-status, expression-status, marker-status, annotation-feature, and environment-key diagnostics; no fitting. |
