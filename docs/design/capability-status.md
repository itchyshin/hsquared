...
| `data_status()` | partial | local tests over `hs_data()` diagnostics | Reports data-container component, ID-overlap, and marker-status diagnostics; no fitting. |
...
| `hs_data()` container | partial | local tests over phenotype, pedigree, genotype, expression, marker-map validation, genotype-marker alignment, ID overlap summaries, marker-status summaries, and v0.1 parser use | No file-backed storage, genotype parsing, marker imputation, marker scanning, or automatic genotype/omics model construction yet. |
