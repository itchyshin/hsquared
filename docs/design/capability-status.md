...
| `data_status()` | partial | local tests over `hs_data()` diagnostics | Reports data-container component, ID-overlap, pedigree-status, marker-status, and environment-key diagnostics; no fitting. |
...
| `hs_data()` container | partial | local tests over phenotype, pedigree, genotype, expression, environment-key diagnostics, pedigree status, marker-map validation, genotype-marker alignment, ID overlap summaries, marker-status summaries, and v0.1 parser use including default pedigree lookup for `animal(1 \| id)` | No file-backed storage, genotype parsing, marker imputation, marker scanning, environmental model terms, or automatic genotype/omics/environment model construction yet. |
