...
| opt-in supplied-variance Henderson MME bridge | partial | local JuliaCall tests compare fixture fixed effects, EBVs, fitted values, and h2 through `hsquared()` with `target = "henderson_mme"` | Hopper/Fisher/Henderson/Rose | Supplied-variance validation path only; no variance-component estimation, log-likelihood, PEV/reliability, or production sparse fitting. |
