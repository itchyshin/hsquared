...
| `hsquared_fit` extractor contract | partial | local tests over mocked result fields, fitted/residual extraction, and opt-in tiny Julia result | Emmy/Pat | PEV/reliability contract exists; `fitted()` and `residuals()` work when normalized predictions and response values are present; the opt-in bridge enriches tiny local Julia results when exported dense validation extractors are available. |
