...
| `hs_data()` input container | partial | local ID-map, marker-map validation, ID-overlap summary, parser integration, and input-shape tests | Emmy/Jason | No large-file, file-backed, PLINK/VCF, genotype parsing, imputation, or marker-scan tests yet. |
