...
| `data_status()` diagnostics | partial | local tests check component, ID-overlap, and marker-status output for `hs_data()` objects | Emmy/Pat/Rose | Diagnostic only; no genotype parsing, marker scanning, or fitting. |
...
| `hs_data()` input container | partial | local ID-map, marker-map validation, genotype-marker alignment, ID-overlap summary, marker-status summary, parser integration, and input-shape tests | Emmy/Jason | No large-file, file-backed, PLINK/VCF, genotype parsing, imputation, or marker-scan tests yet. |
