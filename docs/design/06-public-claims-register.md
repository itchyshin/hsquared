...
| `hsquared` defines the first fitted-object/extractor contract | partial | local tests over internal `hsquared_fit` objects, mocked result fields, EBV/BLUP aliases, derived accuracy, fitted/residual extraction, and opt-in tiny Julia bridge enrichment for PEV/reliability when available | extractor plumbing and tiny dense validation enrichment only; no production sparse PEV/reliability |
