...
| `hsquared` provides a data container for integrated inputs | partial | local tests for `hs_data()` ID maps, input validation, and v0.1 parser use | lightweight data container with phenotype/pedigree parser integration; no file-backed storage or automatic genotype/omics model construction |
