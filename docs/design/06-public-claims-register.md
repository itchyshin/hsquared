...
| `hsquared` provides a data container for integrated inputs | partial | local tests for `hs_data()` ID maps, marker-map validation, ID overlap summaries, input validation, and v0.1 parser use | lightweight data container with phenotype/pedigree parser integration and marker metadata checks; no file-backed storage, genotype parsing, marker scanning, or automatic genotype/omics model construction |
