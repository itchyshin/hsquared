...
| `hsquared` provides a data container for integrated inputs | partial | local tests for `hs_data()` ID maps, pedigree status, expression-status diagnostics, marker-map validation, genotype-marker alignment, annotation-feature diagnostics, environment-key diagnostics, ID overlap and marker-status summaries, input validation, and v0.1 parser use including default pedigree lookup | lightweight data container with phenotype/pedigree parser integration and pedigree/expression/marker/annotation/environment metadata diagnostics; no file-backed storage, genotype parsing, marker scanning, eQTL/omics models, environmental model terms, or automatic genotype/omics/environment model construction |
| `data_status()` reports data-container diagnostics | partial | local tests over `hs_data()` component, ID-overlap, pedigree-status, expression-status, marker-status, annotation-status, and environment-status diagnostics | status helper only; no model fitting, genotype parsing, marker scanning, eQTL/omics models, environmental model terms, pedigree inverse, or relationship-matrix construction |
