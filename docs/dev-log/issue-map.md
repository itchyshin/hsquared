...
| --- | --- | --- | --- | --- | --- |
| 5 | R fitted object & extractor contract | 1 | r-package | partial | extractor contract; scan-object `gwas_table(scan)` / `lod_scores(scan)` banked; HSquared.jl PR #148 (`b6345f1`) now mirrors that R status wording in Julia source docs/public-claims after R #85/#86/#87. Fit-level map-annotated QTL/GWAS/eQTL tables, lowrank/fa loading outputs, metafounder effects, and production PEV/reliability remain gated by #22/#23 and status rows |
| 6 | R-to-Julia bridge payload design | 1 | bridge | partial | **bridge epic**; parent remains open while #22/#23 and twin-gated result-shape contracts stay active. Banked surfaces include the default Gaussian bridge, experimental opt-in engine targets, standard PEV/reliability consumption when present, the Julia PR #142 marker-scan payload fixture mirrored by R, R PR #82 scan-object `gwas_table(scan)` / `lod_scores(scan)`, the Julia PR #140 genomic GBLUP/SNP-BLUP target mirrored by R PR #84, and Julia PR #147 recording that R consumer check in the comparator-target manifest/status docs. These are bridge/fixture/status handoffs only, not external comparator evidence or covered promotion. |
...
| 10 | Multivariate validation: comparator & recovery gates | 3 | validation | partial | V4-MULTIVARIATE/V4-MV-REML (partial); R evidence legs now include cold-start recovery, full-unstructured `sommer`, Mrode Example 5.1 supplied-G0/R0 BLUP/MME, and a Bayesian `MCMCglmm` agreement probe; HSquared.jl PR #151 (`c25bcc1`) hardens the BLUPF90/AIREMLF90 numeric packet/output-alignment setup only; twin #41/#49 still own promotion gates |
...
| 23 | Bridge: post-fit `gwas()`/marker-scan wrapper | 5 | bridge | partial | Mixed, single-marker, and LOCO scan bridges are live and experimental; `gwas_table(scan)` and `lod_scores(scan)` now expose thin views of an already-computed `hs_gwas` object. Julia PR #142 (`f9fbbb1`) closed #45 by exporting a stable row-aligned `marker_scan_result_payload()` and serialized fixture, now mirrored by an R Julia-free payload-normalization test; Julia PR #146 (`a350225`) mirrored the R scan-result table status in Julia docs/status only. Julia PR #134 banked a fixed-panel calibration smoke harness, PR #135 reopened #48 as the active calibration/evidence gate, and PR #143 (`07a3c63`) synced the Julia `V5-MARKER-THRESHOLD` status row while keeping #48 open. Local R scan-tool availability is blocked (`2026-06-21-marker-scan-tool-availability.md`: no PLINK/GEMMA/GCTA/SAIGE executables; no GenABEL/qvalue/rrBLUP/BGLR/AGHmatrix packages; `sommer` only). R significance thresholds remain inactive. Future gates are permutation/realistic-LD threshold activation, external comparator/calibration evidence, formula-level scan grammar, and map-annotated QTL/eQTL result tables (active twin #48/#61, with #45 closed). |
| 24 | Innovation: augmented AI-REML single-solve (Strandén 2024) | 8 | innovation/perf | planned | engine-led; twin #58; live issue body now pins this as a planned Gaussian REML/AI-REML restructuring only, gated on identical variance components/logLik/AI-step behaviour vs standard AI-REML before any benchmark or speed claim. HSquared.jl PR #150 (`23eced6`) mirrored the planned-only gate into live Julia #58. |
| 25 | Innovation: SQUAREM EM accelerator (engine utility) | — | innovation/perf | planned | engine-led; twin #58; live issue body now pins this as a planned EM/fixed-point utility only, gated on same fixed point plus guarded/backtracking objective behaviour before any iteration/time claim. HSquared.jl PR #150 (`23eced6`) mirrored the planned-only gate into live Julia #58. |
...
| --- | --- | --- | --- | --- |
| 48 | Validation: calibrated genome-wide significance thresholds | 5 | open · partial | R #23; PR #134 banked fixed-panel smoke, PR #135 reopened #48 as the active evidence gate, PR #143 (`07a3c63`) synced the Julia validation-status/source-doc row, and PR #148 (`b6345f1`) recorded the R #83 marker-scan comparator/threshold tool-availability blocker in live Julia #48; R #59/#60 banked the activation contract and inert metadata validator, but no R threshold is active |
...
| 58 | Engine perf ideas (augmented AI-REML / SQUAREM / Woodbury) | 8 | planned · cross-lane | hsquared#24/#25/#17; Julia PR #150 (`23eced6`) refreshed live #58 after R #91 with planned-only gates for augmented AI-REML, SQUAREM, and Woodbury helpers. No engine implementation, benchmark/speedup claim, comparator evidence, or promotion follows from the issue sync |
...
| 6 | Engine result object and diagnostics | 1 | partial | hsquared#6; Julia PR #149 (`bcdcd4c`) refreshed the live Julia #6 parent ledger after R #89 |
| 7 | Julia-side validation canon | 1 | partial | hsquared#7/#10; Julia PR #149 (`bcdcd4c`) refreshed the live Julia #7 parent ledger after R #88 |
...
| 44 | Bridge activation: non-Gaussian LA/VA + MarginalMethod | 6 | partial · cross-lane | hsquared#18 closed; HSquared.jl PR #152 (`3843ddb`) banked the `non_gaussian_parity` payload fixture and the R mirror now consumes it in Julia-free normalizer tests, including Poisson-Laplace, Binomial-variational, canonical LA/VA method normalization, no-heritability, and serialized vector-`n_trials` preservation. HSquared.jl PR #154 (`38286b1`) corrected Julia #44/ledger wording after R PR #96 (`e7c7a4a`): the remaining R bridge gap is per-record varying-trial activation, not absent non-Gaussian formula/live bridge support wholesale. No external comparator, no interval calibration, and no covered-status promotion. |
...
| Non-Gaussian LA/VA | #18 closed | #44 (+ #40) | R already has the opt-in Poisson/Bernoulli/Binomial common-trial bridge, LA/VA marginal control, and no-heritability result contract; HSquared.jl PR #152 (`3843ddb`) adds the `NonGaussianFit` payload fixture, mirrored by R PR #95 (`05fbdd3`) normalizer tests with vector-`n_trials` preservation; R PR #96 (`e7c7a4a`) and HSquared.jl PR #154 (`38286b1`) reconciled the bridge-gap wording on both sides. Remaining gates are per-record varying-trial formula/bridge activation, non-Gaussian recovery/calibration, external GLLVM/gllvmTMB/MCMCglmm-or-equivalent comparator evidence, and any promotion beyond partial. |
| Marker scans | #23 | #45 closed + #48 (+ PR #134 smoke, PR #135 reopen, PR #143 status row, PR #146 R-table sync, PR #148 R extractor/blocker sync) | Phase 5 scan payload fixture banked by Julia PR #142 and mirrored by R payload-normalization parity; R now has thin `gwas_table(scan)` / `lod_scores(scan)` views for existing `hs_gwas` results, mirrored into Julia docs/status by PR #146/#148 only; local scan-comparator tools are unavailable on this host; fixed-panel threshold smoke and Julia validation-status hygiene are banked and #48 remains the active evidence gate, but no R significance threshold activation, formula-level scan grammar, map-annotated table workflow, or production calibration claim |
...
- HSquared.jl PR #151 (`c25bcc1`) hardened the BLUPF90/AIREMLF90 multivariate
  starter packet to emit numeric BLUPF90-ready data, integer-coded pedigree
  rows, an animal ID map, and matching RENUMF90 records. It did not run a
  BLUPF90-family executable, align comparator estimates, or promote
...
- HSquared.jl PR #146 (`a350225`) synced Julia docs/status/issue text after R
  PR #82 so the Julia lane also records `gwas_table(scan)` and
  `lod_scores(scan)` as thin views of existing R `hs_gwas` objects. It is
