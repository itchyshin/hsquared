...
| 44 | Bridge activation: non-Gaussian LA/VA + MarginalMethod | 6 | partial · cross-lane | hsquared#18 closed; HSquared.jl PR #152 (`3843ddb`) banked the `non_gaussian_parity` payload fixture and the R mirror now consumes it in Julia-free normalizer tests, including Poisson-Laplace, Binomial-variational, canonical LA/VA method normalization, no-heritability, and serialized vector-`n_trials` preservation. No R formula activation for per-record varying trials, no external comparator, no interval calibration, and no covered-status promotion. |
...
| Non-Gaussian LA/VA | #18 closed | #44 (+ #40) | R already has the opt-in Poisson/Bernoulli/Binomial common-trial bridge, LA/VA marginal control, and no-heritability result contract; HSquared.jl PR #152 (`3843ddb`) adds the `NonGaussianFit` payload fixture, now mirrored by R normalizer tests with vector-`n_trials` preservation. Remaining gates are per-record varying-trial formula/bridge activation, non-Gaussian recovery/calibration, external GLLVM/gllvmTMB/MCMCglmm-or-equivalent comparator evidence, and any promotion beyond partial. |
