...
`validation_status()` in the installed package is the live source of truth. This summary is generated from it, so a status change here is a status change there - the two cannot drift apart silently.
...
1. Run `validation_status()` and confirm your route's row still says what this page says.
