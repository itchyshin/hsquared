...
v07_test_inputs <- function(tier = "confirm") {
  ...
  raw <- data.frame(
    ...
    converged = c(TRUE, TRUE, TRUE, TRUE, FALSE),
    iterations = c(8, 8, 9, 8, -1),
    objective = c(1, 2, 3, 4, NA),
    gradient_norm = c(1e-8, 1e-8, 1e-8, 1e-8, NA),
    runtime_seconds = 1,
    peak_rss_mb = 10,
    marker_hash = vapply(
      seeds,
      function(i) v07_test_hash(as.character(i)),
      character(1)
    ),
    id_hash = v07_test_hash("a"),
    kernel_hash = vapply(
      seeds,
      function(i) v07_test_hash(letters[i]),
      character(1)
    ),
...
test_that("raw reader accepts real one-row TSV data frames", {
  ...
  for (i in 1:2) {
    utils::write.table(
      x$raw[i, , drop = FALSE],
      file.path(raw_dir, paste0(i, ".tsv")),
      sep = "\t",
      quote = FALSE,
      row.names = FALSE
    )
  ...
  utils::write.table(
    x$raw[1:2, , drop = FALSE],
    file.path(raw_dir, "bad.tsv"),
    sep = "\t",
    quote = FALSE,
    row.names = FALSE
  )
...
test_that("R versus Julia comparison uses a 1e-10 numeric gate", {
  ...
  expect_error(
    v07_compare_summary(julia, outside, 1e-10),
...
test_that("every preregistered mutation turns at least one gate red", {
  ...
  expect_error(
    v07_validate_inputs(truth, x$manifest, "confirm", expected),
    "raw/manifest mismatch"
  )

  duplicate <- rbind(x$raw, x$raw[1, ])
  expect_error(
    v07_validate_inputs(duplicate, x$manifest, "confirm", expected),
    "duplicate"
  )

  removed_failed <- x$raw[x$raw$converged, ]
  expect_error(
    v07_validate_inputs(removed_failed, x$manifest, "confirm", expected),
    "exactly equal"
  )
  ...
  expect_error(
    v07_validate_inputs(x$raw, cell_label, "confirm", expected),
    "cell labels"
  )
  ...
  ridge$ridge[[1]] <- 0.02
  expect_error(
    v07_validate_inputs(x$raw, ridge, "confirm", expected),
    "frozen ridge"
  )
  ...
  expect_error(
    v07_validate_inputs(raw_ridge, x$manifest, "confirm", expected),
    "raw/manifest mismatch"
  )
  ...
  expect_error(
    v07_validate_inputs(marker_hash, x$manifest, "confirm", expected),
    "SHA-256"
  )
  ...
  expect_error(
    v07_validate_inputs(id_order, x$manifest, "confirm", expected),
    "ID-order"
  )
